  ...
  <div>
    ...
    <el-dialog title="修改订单" :visible.sync="editVisible" width="50%">
      <el-form ref="editForm" :model="editForm" label-width="150px">
        ...
        <el-form-item label="子模块" prop="submodel">
          <el-select v-model="editForm.submodel" multiple>
            <el-option
              v-for="item in numericalManageTable.list"
              :label="item.name"
              :value="item.name"
              :key="item.name"
            ></el-option>
    ...
    <el-dialog title="新增订单" :visible.sync="insertVisible" width="50%">
      <el-form ref="insertForm" :model="insertForm" label-width="150px">
        ...
        <el-form-item label="子模块" prop="submodel">
          <el-select v-model="insertForm.submodel" multiple>
            <el-option
              v-for="item in numericalManageTable.list"
              :label="item.name"
              :value="item.name"
...
<script>
import api from "@api/EngineeringComputingModuleTestCaseManagement";
export default {
  components: {},
  props: {},
  data() {
  ...
  methods: {
    getData() {
      let data = JSON.parse(localStorage.getItem("engineeringManageTable"));
    ...
    saveInsert() {
      this.insertForm["fileUrl"] =
        "https://richardzpc-oss.oss-cn-beijing.aliyuncs.com/EngTest3.xlsx";
      this.insertForm["checkpoints"] =
        this.insertForm["content"].split("\n").length / 2;
      this.insertForm["subCount"] = this.insertForm["submodel"].length;
  ...
  created() {
    this.getData();
    this.numericalManageTable = JSON.parse(
      localStorage.getItem("numericalManageTable")
    );
